...
export function AdminStudio() {
  ...
  if (!user || !session) return <div className="admin-login"><div className="admin-login__panel"><Image src="/lfx-mark.svg" alt="HiPER" width={90} height={90}/><span className="eyebrow">HiPER Studio</span><h1>Selamat Datang, Pentadbir!</h1><p>{language === "bm" ? "Log masuk menggunakan akaun DELIMa Pentadbir." : "Sign in using the Administrator's DELIMa account."}</p><GoogleAuth onUser={setUser}/>{checking && <p className="muted">Verifying role…</p>}<FlashMessage flash={flash}/></div></div>;
