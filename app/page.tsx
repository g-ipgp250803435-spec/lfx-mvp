...
export default function HomePage() {
  ...
  return <>
    ...
    <section className="section"><div className="container"><div className="section-heading-row"><div className="section-heading"><span className="eyebrow">Pengumuman</span><h2>{language === "bm" ? "Makluman rasmi, tersusun dan mudah dikongsi." : "Official notices, organised and shareable."}</h2></div><Link href="/announcements" className="text-link">{labels.viewAll}<Icon name="arrow" size={16}/></Link></div><AnnouncementList compact/></div></section>
    <section className="cta-section"><div className="container cta-section__inner"><div><span className="eyebrow brand-on-dark">Hab Perbendaharaan Digital</span><h2>{t(content.site.tagline, language)}</h2><p>{t(content.site.description, language)}</p></div><Link href="/organisation" className="button button--accent">{language === "bm" ? "Kenali pejabat kami" : "Meet our office"}<Icon name="arrow" size={17}/></Link></div></section>
  </>;
}
